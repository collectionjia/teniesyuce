...
async function start() {
  if (started) return;
  started = true;
  if (String(process.env.SCHEDULER_LOOP_ENABLED || '1') === '0') {
    console.log('[scheduler] loop disabled (SCHEDULER_LOOP_ENABLED=0)');
    return;
  }
  try {
  ...
  console.log('[scheduler] loop started (1s poll, executor=%s)', process.env.EXECUTOR_MODE || 'http');
...
async function status() {
  ...
  return {
    running: started,
    executorMode: process.env.EXECUTOR_MODE || 'http',
