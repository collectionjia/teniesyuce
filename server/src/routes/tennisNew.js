...
/** @deprecated 转发盘前：new → tennis-prematch */
const router = Router();
...
router.post('/cache/refresh', auth(['admin']), async (_req, res) => {
  res.json({ ok: true, deprecated: true, message: '请改用全量采集写入 tennis:bundle:prematch' });
});
...
router.post('/trade/batch', auth(), requireWallet, async (req, res) => {
  try {
    ...
    console.error('[tennis-new/trade/batch→prematch]', e);
