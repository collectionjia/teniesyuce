# 本地连线上 MySQL + 生产 Redis（SSH 隧道 9003 / 9004）
# 用法（仓库根目录）:
...
Write-Host '==> SSH tunnels: Redis 9003, Sofa 9004' -ForegroundColor Cyan
Stop-LocalPort 9003
Stop-LocalPort 9004
...
$tunnel = Start-Job -Name 'yuce-tunnel' -ScriptBlock {
  ...
  & ssh -i $KeyPath -o StrictHostKeyChecking=no -o ServerAliveInterval=30 -N `
    -L 127.0.0.1:9003:127.0.0.1:9003 `
...
if ($raw -notmatch '(?m)^REDIS_URL=') {
  $raw = $raw.TrimEnd() + "`r`nREDIS_URL=redis://127.0.0.1:9003`r`n"
} else {
  $raw = [regex]::Replace($raw, '(?m)^REDIS_URL=.*$', 'REDIS_URL=redis://127.0.0.1:9003')
}
...
$redisOk = $false
for ($i = 0; $i -lt 10; $i++) {
  node -e "const {createClient}=require('redis');(async()=>{const c=createClient({url:'redis://127.0.0.1:9003'});await c.connect();console.log('redis',await c.ping(),'keys',(await c.keys('tennis:*')).length);await c.quit()})().catch(e=>{console.error(e.message);process.exit(1)})" 2>$null
...
if (-not $redisOk) {
  Pop-Location
  throw 'Redis tunnel failed — check server port 9003'
}
Pop-Location
...
Write-Host "  Redis     127.0.0.1:9003 -> $HostName`:9003 (prod tennis bundles)"
