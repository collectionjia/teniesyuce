#!/bin/bash
set -e
cd /opt/yuce/scripts/tennis-monitor
...
sudo cp /tmp/tennisFromMonitor.js /opt/yuce/server/src/services/tennisFromMonitor.js
docker cp /opt/yuce/server/src/services/tennisFromMonitor.js yuce-server-1:/app/src/services/tennisFromMonitor.js
cd /opt/yuce
docker compose -f docker-compose.core.yml restart server
sleep 4
docker exec yuce-server-1 node /app/refresh_tennis_redis.js
