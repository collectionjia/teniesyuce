...
# ── 多市场配置 ─────────────────────────────────────────────────────────────────
MARKETS = {
    # 展示用「当前价格」= Chainlink 现货；结算 TWAP60 另存 rtds 缓存
    "btc_5m":  {"label": "BTC 5m",  "slug_prefix": "btc-updown-5m",  "duration": 300,  "symbol": "BTC", "variant": "fiveminute",    "rtds": "btc/usd_chainlink"},
    "btc_15m": {"label": "BTC 15m", "slug_prefix": "btc-updown-15m", "duration": 900,  "symbol": "BTC", "variant": "fifteenminute", "rtds": "btc/usd_chainlink"},
    "btc_1h":  {"label": "BTC 1h",  "slug_prefix": "bitcoin",        "duration": 3600, "symbol": "BTC", "variant": "onehour",       "rtds": "btc/usd_chainlink", "slug_type": "hourly"},
...
state = {
    ...
    "up_book": {"bids": [], "asks": []},
    "down_book": {"bids": [], "asks": []},
    # 看板用: 5m/15m/1h UP·DOWN 盘口价
    "board_prices": {
        "5m": {"up": 0.0, "down": 0.0, "up_ask": 0.0, "down_ask": 0.0},
        "15m": {"up": 0.0, "down": 0.0, "up_ask": 0.0, "down_ask": 0.0},
        "1h": {"up": 0.0, "down": 0.0, "up_ask": 0.0, "down_ask": 0.0},
    },
}
# RTDS 推送缓存: symbol -> price（官网现价用 btcusdt）
_rtds_prices = {
    "btc/usd": 0.0,
    "eth/usd": 0.0,
    "btcusdt": 0.0,
...
def state_worker():
    ...
    while True:
        try:
            ...
            # 更新 RTDS → state.crypto_current（BTC 展示用 Chainlink 现货，对齐官网「当前价格」）
            rtds_key = mkt["rtds"]
            with state_lock:
                px = float(_rtds_prices.get(rtds_key) or 0)
                if px <= 0 and mkt_key.startswith("btc_"):
                    px = float(
                        _rtds_prices.get("btc/usd_chainlink")
                        or _rtds_prices.get("btc/usd_twap60")
                        or _rtds_prices.get("btc/usd_twap30")
                        or _rtds_prices.get("btcusdt")
                        or _rtds_prices.get("btc/usd")
                        or 0
                    )
                elif px <= 0:
                    px = float(_rtds_prices.get("btcusdt") or _rtds_prices.get("btc/usd") or 0)
                # 嵌入模式下 onchain 已持续写入 crypto_current，非 0 时不回退覆盖成 0
                if px > 0:
                    state["crypto_current"] = px

            now = int(time.time())
            round_ts = now - (now % duration)
            strike_pending = False

            # 嵌入看板时 5m 回合/锚定价由 onchain_leaderboard 写入 state
            if not (_embedded and mkt_key == "btc_5m"):
                # ── 新回合检测 ──
    ...
    while True:
        try:
            with state_lock:
                tids = dict(state["token_ids"])
                if not tids:
                    tids = dict(state.get("_lb_tids_5m") or {})
            ...
            # 看板 5m/15m/1h 盘口价 (~0.5s)
            board_tick += 1
            if board_tick >= 5:
                board_tick = 0
                with active_market_lock:
                    cur_mkt = active_market
                with state_lock:
                    tf_map = {
                        "5m": state.get("_lb_tids_5m") or (
                            dict(tids) if cur_mkt == "btc_5m" else {}
                        ),
                        "15m": state.get("_lb_tids_15m") or {},
                        "1h": state.get("_lb_tids_1h") or {},
                    }
                prices = {}
                for tf, tf_tids in tf_map.items():
                    if not tf_tids:
                        continue
                    up_bid, up_ask = _best_prices(tf_tids.get("up"))
                    dn_bid, dn_ask = _best_prices(tf_tids.get("down"))
                    prices[tf] = {
                        "up": up_bid, "down": dn_bid,
                        "up_ask": up_ask, "down_ask": dn_ask,
                    }
                if prices:
                    with state_lock:
                        bp = dict(state.get("board_prices") or {})
                        bp.update(prices)
                        state["board_prices"] = bp
        except Exception:
            pass
...
def _board_rounds_snapshot():
    """Per-timeframe round window + strike/current for board API consumers."""
    now = int(time.time())
    durations = {"5m": 300, "15m": 900, "1h": 3600}
    strikes = {"5m": 0.0, "15m": 0.0, "1h": 0.0}
    currents = {"5m": 0.0, "15m": 0.0, "1h": 0.0}
    # Prefer quick_trade.state (same process), then onchain module prices.
    # Note: when board runs as `python onchain_leaderboard.py`, price threads update
    # __main__._btc_prices while `import onchain_leaderboard` is a second module copy.
    with state_lock:
        strikes["5m"] = float(state.get("strike") or 0)
        strikes["15m"] = float(state.get("strike_15m") or 0)
        strikes["1h"] = float(state.get("strike_1h") or 0)
        currents["5m"] = float(state.get("crypto_current") or 0)
        currents["15m"] = float(state.get("crypto_current_15m") or 0)
        currents["1h"] = float(state.get("crypto_current_1h") or 0)
        s5_ts = int(state.get("round_ts") or 0)
        s5_end = int(state.get("round_end") or 0)
    try:
        import sys
        olb = sys.modules.get("onchain_leaderboard")
        main = sys.modules.get("__main__")
        for mod in (olb, main):
            if not mod or not hasattr(mod, "_btc_prices"):
                continue
            lock = getattr(mod, "_btc_price_lock", None)
            prices = getattr(mod, "_btc_prices", {})
            if lock is None:
                continue
            with lock:
                if not strikes["5m"]:
                    strikes["5m"] = float(prices.get("strike") or 0)
                if not strikes["15m"]:
                    strikes["15m"] = float(prices.get("strike_15m") or 0)
                if not strikes["1h"]:
                    strikes["1h"] = float(prices.get("strike_1h") or 0)
                if not currents["5m"]:
                    currents["5m"] = float(prices.get("current") or 0)
                if not currents["15m"]:
                    currents["15m"] = float(prices.get("current_15m") or 0)
                if not currents["1h"]:
                    currents["1h"] = float(prices.get("current_1h") or 0)
    except Exception:
        pass
    rounds = {}
    for tf, dur in durations.items():
        ...
        cur = currents.get(tf, 0.0) or currents.get("5m", 0.0)
        # All timeframes share the same live spot; prefer 5m/crypto_current.
        if tf != "5m" and currents.get("5m", 0.0) > 0:
            cur = currents["5m"]
        rounds[tf] = {
            "round_ts": ts,
            "round_end": end,
...
def _snapshot_state():
    with state_lock:
        data = dict(state)
        # nested dicts need shallow copies for JSON safety
        if isinstance(data.get("board_prices"), dict):
            data["board_prices"] = {
                k: dict(v) if isinstance(v, dict) else v
                for k, v in data["board_prices"].items()
            }
        for key in ("leaderboard", "leaderboard_15m", "leaderboard_1h"):
    ...
    for drop in ("up_book", "down_book", "trades", "accounts", "token_ids",
                 "_lb_tids_5m", "_lb_tids_15m", "_lb_tids_1h"):
        data.pop(drop, None)
    return data


async def board_state_api(request):
    """HTTP 轮询接口 — iframe 嵌入时 WebSocket 无法升级，用此接口"""
    return web.json_response(_snapshot_state())


async def crawl_api(request):
    """GET/POST /api/crawl — 读取或设置链上爬取开关"""
    import onchain_leaderboard as olb
    ...
    if "enabled" not in data:
        return web.json_response({"success": False, "message": "缺少 enabled"}, status=400)
    enabled = bool(data.get("enabled"))
    olb.set_crawl_enabled(enabled)
    return web.json_response({"success": True, "enabled": olb.is_crawl_enabled()})
...
async def winner_records_get_api(request):
    import onchain_leaderboard as olb
    return web.json_response({"success": True, "records": olb.load_winner_records()})
...
app.router.add_get("/board", board_page)
app.router.add_get("/trade", trade_page)
...
app.router.add_get("/api/winner-records", winner_records_get_api)
...
.lb-row.streak-row { color: #ff6b35; }
.lb-hot-section {
    border-bottom: 1px solid var(--border);
...
.lb-hot-header:hover { background: rgba(255,255,255,0.03); }
.lb-hot-header-left {
    display: flex;
    align-items: center;
    gap: 8px;
...
.lb-hot-section.collapsed .lb-hot-toggle { transform: rotate(-90deg); }
.lb-hot-title {
    color: #ff6b35;
    font-weight: 700;
    font-size: 12px;
...
.lb-hot-section.collapsed .lb-hot-body {
    max-height: 0;
    opacity: 0;
    padding-top: 0;
    padding-bottom: 0;
...
.lb-round-invest-row:last-child {
    margin-bottom: 0;
}
.lb-round-invest-label {
    font-size: 12px;
    color: var(--text-muted);
    font-family: 'JetBrains Mono', monospace;
    margin-bottom: 3px;
...
.lb-round-count.up { color: var(--up); }
.lb-round-count.dn { color: var(--down); }
.lb-round-pct {
    font-size: 11px;
    color: var(--text-muted);
}
...
.lb-ratio-bar.sm { height: 6px; }
.lb-ratio-up { background: var(--up); transition: width 0.3s; }
.lb-ratio-dn { background: var(--down); transition: width 0.3s; }
.lb-hot-item {
    padding: 2px 8px;
    border-radius: 6px;
...
.lb-hot-item a { color: inherit; text-decoration: none; }
.lb-hot-item a:hover { color: var(--blue); text-decoration: underline; }
.lb-record-section {
    border-bottom: 1px solid var(--border);
...
.lb-record-header:hover { background: rgba(255,255,255,0.03); }
.lb-record-header-left {
    display: flex;
    align-items: center;
    gap: 8px;
    min-width: 0;
...
.lb-record-section.collapsed .lb-record-toggle { transform: rotate(-90deg); }
.lb-record-title {
    font-size: 12px;
    font-weight: 700;
    color: var(--text);
}
.lb-record-header-actions {
    display: flex;
    align-items: center;
    gap: 8px;
    flex-shrink: 0;
...
.lb-record-btn:hover { background: rgba(59, 130, 246, 0.25); }
.lb-record-btn:disabled { opacity: 0.5; cursor: not-allowed; }
.lb-record-body {
    padding: 0 16px 12px;
    overflow: hidden;
    transition: max-height 0.25s ease, opacity 0.2s ease;
...
.lb-record-section.collapsed .lb-record-body {
    max-height: 0;
    opacity: 0;
    padding-top: 0;
    padding-bottom: 0;
...
.lb-record-section.collapsed .lb-record-summary-inline { display: inline; }
.lb-record-section:not(.collapsed) .lb-record-summary-inline { display: none; }
.lb-record-table-wrap {
    max-height: 220px;
    overflow: auto;
    border: 1px solid var(--border);
    border-radius: 8px;
...
.lb-record-table tr:last-child td { border-bottom: none; }
.lb-record-table .win { color: var(--up); font-weight: 700; }
.lb-record-table .lose { color: var(--down); font-weight: 700; }
.lb-record-table .pending { color: var(--text-muted); }
...
<div class="app">
    ...
    <div class="lb-panel" id="lb-panel" style="display:none">
        ...
        <div class="lb-round-invest" id="lb-round-invest" style="display:none">
            <div class="lb-round-invest-title" id="lb-round-invest-title">📊 本轮投入</div>
            <div class="lb-round-invest-row">
                <div class="lb-round-invest-label" id="lb-round-people-label">人数 —</div>
                <div class="lb-ratio-bar">
                    <div class="lb-ratio-up" id="lb-round-people-up" style="width:50%"></div>
                    <div class="lb-ratio-dn" id="lb-round-people-dn" style="width:50%"></div>
                </div>
            </div>
        </div>
        <div class="lb-hot-section collapsed" id="lb-hot-section" style="display:none">
            <div class="lb-hot-header" onclick="toggleLbHotStreak()">
                <div class="lb-hot-header-left">
                    <span class="lb-hot-toggle">▼</span>
                    <span class="lb-hot-title" id="lb-hot-title">🔥 连胜冒火</span>
                </div>
            </div>
            <div class="lb-hot-summary">
                <div class="lb-hot-ratio" id="lb-hot-ratio"></div>
                <div class="lb-hot-ratio-bar">
                    <div class="lb-hot-ratio-up" id="lb-hot-ratio-up" style="width:50%"></div>
                    <div class="lb-hot-ratio-dn" id="lb-hot-ratio-dn" style="width:50%"></div>
                </div>
            </div>
            <div class="lb-hot-body">
                <div class="lb-hot-streak-items" id="lb-hot-streak-items"></div>
            </div>
        </div>
        <div class="lb-record-section collapsed" id="lb-record-section">
            <div class="lb-record-header" onclick="toggleLbRecords()">
                <div class="lb-record-header-left">
                    <span class="lb-record-toggle" id="lb-record-toggle">▼</span>
                    <span class="lb-record-title">🏅 上轮赢家跟随记录</span>
                    <span class="lb-record-summary" id="lb-record-summary-inline">每局结算后自动记录</span>
                </div>
            </div>
            <div class="lb-record-body" id="lb-record-collapse">
                <div class="lb-record-summary" id="lb-record-summary">每局结算后自动记录</div>
                <div class="lb-record-table-wrap">
                    <table class="lb-record-table">
                        <thead>
                            <tr>
                                <th>回合</th>
                                <th>上轮赢家</th>
                        ...
                        <tbody id="lb-record-tbody">
                            <tr><td colspan="7" class="pending">暂无记录 — 每局结算后自动写入</td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
...
let _lbWinnersCache = null;  // 上轮赢家本轮分布 (5m)
...
function updateUI(data) {
    ...
    if (data.winner_records) {
        renderWinnerRecords(data.winner_records);
    }
...
function renderLeaderboard(lb) {
    ...
    if (lb.hedged_count > 0) {
        const label = lb.filter_makers ? '🚫 已过滤做市商' : '⚖️ 做市商';
        statsHtml += `<span class="lb-stat-tag">${label} ${lb.hedged_count}人</span>`;
    }
    if (lb.winners) {
        const w = lb.winners;
        const net = w.net_shares != null ? w.net_shares : Math.abs(w.up_s - w.dn_s);
        const dir = w.net_dir || (w.up_s > w.dn_s ? 'UP' : w.dn_s > w.up_s ? 'DN' : '--');
        const arrow = dir === 'UP' ? '🟢' : dir === 'DN' ? '🔴' : '⚪';
        const maj = w.majority_side ? ` | 人多:${w.majority_side}` : '';
        statsHtml += `<span class="lb-stat-tag">🏅 上轮${w.side.toUpperCase()}赢家 ${w.total}人 | 本轮: ${w.up_n}人UP(${w.up_s.toLocaleString()}股) vs ${w.dn_n}人DN(${w.dn_s.toLocaleString()}股) | ${arrow} 净${dir} ${net.toLocaleString()}股${maj}</span>`;
    }
    if (lb.streak4) {
        const s4 = lb.streak4;
    ...
    const hotSection = document.getElementById('lb-hot-section');
    if (lb.hot_streaks && lb.hot_streaks.length) {
        const hs = lb.hot_streak_stats || {};
        const upPct = hs.up_pct || 0;
        const dnPct = hs.dn_pct || 0;
        const upSharePct = hs.up_share_pct || 0;
        const dnSharePct = hs.dn_share_pct || 0;
        const ratioArrow = (hs.up_n || 0) > (hs.dn_n || 0) ? '🟢' : (hs.dn_n || 0) > (hs.up_n || 0) ? '🔴' : '⚪';
        const ratioHtml =
            `${ratioArrow} <span class="lb-round-count up">${hs.up_n || 0}人UP</span> <span class="lb-round-pct">(${upPct}%)</span>`
            + ` vs <span class="lb-round-count dn">${hs.dn_n || 0}人DN</span> <span class="lb-round-pct">(${dnPct}%)</span>`
            + ` <span class="lb-round-pct">| 股比 UP ${upSharePct}% / DN ${dnSharePct}%</span>`;
        const items = lb.hot_streaks.map(h => {
            const short = h.addr.slice(0, 6) + '…' + h.addr.slice(-4);
            const name = h.name ? ` (${h.name})` : '';
            const sideCls = h.side === 'UP' ? 'up-col' : 'dn-col';
            return `<span class="lb-hot-item ${sideCls}">🔥${h.count} <a href="https://polymarket.com/profile/${h.addr}" target="_blank" title="${h.addr}">${short}${name}</a> ${h.side} ${Math.round(h.shares).toLocaleString()}股</span>`;
        }).join('');
        document.getElementById('lb-hot-title').textContent = `🔥 连胜冒火 (${lb.hot_streaks.length}人)`;
        document.getElementById('lb-hot-ratio').innerHTML = ratioHtml;
        document.getElementById('lb-hot-ratio-up').style.width = `${upPct}%`;
        document.getElementById('lb-hot-ratio-dn').style.width = `${dnPct}%`;
        document.getElementById('lb-hot-streak-items').innerHTML = items;
        hotSection.style.display = '';
    } else {
        document.getElementById('lb-hot-streak-items').innerHTML = '';
        hotSection.style.display = 'none';
    }

    renderRoundInvest(lb);
...
function renderRoundInvest(lb) {
    const panel = document.getElementById('lb-round-invest');
    if (lb.winners) _lbWinnersCache = lb.winners;
    const w = lb.winners || _lbWinnersCache;
    if (!w || (w.up_n == null && w.dn_n == null)) {
    ...
    const dnN = w.dn_n || 0;
    const total = upN + dnN;
    const upPct = total ? Math.round(upN / total * 1000) / 10 : 0;
    const dnPct = total ? Math.round(dnN / total * 1000) / 10 : 0;
    const pArrow = upN > dnN ? '🟢' : dnN > upN ? '🔴' : '⚪';
    const sideLabel = w.side ? w.side.toUpperCase() : '?';
    document.getElementById('lb-round-invest-title').textContent =
        `📊 本轮投入 · 上轮${sideLabel}赢家 ${w.total || 0}人`;
    document.getElementById('lb-round-people-label').innerHTML =
        `${pArrow} 本轮 <span class="lb-round-count up">${upN}人UP</span> <span class="lb-round-pct">(${upPct}%)</span>`
        + ` vs <span class="lb-round-count dn">${dnN}人DN</span> <span class="lb-round-pct">(${dnPct}%)</span>`;
    document.getElementById('lb-round-people-up').style.width = `${upPct}%`;
    document.getElementById('lb-round-people-dn').style.width = `${dnPct}%`;
}
...
function renderLbCol(entries, total) {
    ...
    return entries.map((e, i) => {
        ...
        const tags = (e.whale ? '🐋' : '') + (e.hedged ? '⚖️' : '') + (e.streak ? `🔥${e.streak_count || ''}` : '');
...
function renderWinnerRecords(records) {
    const body = document.getElementById('lb-record-tbody');
    const summary = document.getElementById('lb-record-summary');
    const summaryInline = document.getElementById('lb-record-summary-inline');
    const setSummary = (text) => {
        if (summary) summary.textContent = text;
        if (summaryInline) summaryInline.textContent = text;
    };
    if (!records || !records.length) {
        body.innerHTML = '<tr><td colspan="7" class="pending">暂无记录 — 每局结算后自动写入</td></tr>';
        setSummary('每局结算后自动记录');
        return;
    }
    const settled = records.filter(r => r.majority_won != null);
    const wins = settled.filter(r => r.majority_won === true).length;
    const losses = settled.filter(r => r.majority_won === false).length;
    const rate = settled.length ? Math.round(wins / settled.length * 100) : 0;
    setSummary(`已记录 ${records.length} 局 | 上轮赢家「人多」一侧押中 ${wins}/${settled.length} (${rate}%)`);

    body.innerHTML = records.map(r => {
        const dist = `${r.up_n}人UP / ${r.dn_n}人DN`;
        const net = r.net_dir ? `净${r.net_dir} ${Number(r.net_shares || 0).toLocaleString()}股` : '--';
        const maj = r.majority_side === 'TIE' ? '持平' : (r.majority_side === 'DN' ? 'DOWN' : r.majority_side || '--');
        const result = r.actual_winner || '待定';
        let outcome = '待定';
        let cls = 'pending';
        if (r.majority_won === true) { outcome = '✅ 中'; cls = 'win'; }
        else if (r.majority_won === false) { outcome = '❌ 未中'; cls = 'lose'; }
        else if (r.majority_side === 'TIE' && r.actual_winner) { outcome = '⚪ 持平'; cls = 'pending'; }
        const prev = r.prev_side ? `${r.prev_side.toUpperCase()} ${r.prev_total}人` : '--';
        return `<tr>
            <td>${r.round_time || '--'}</td>
            <td>${prev}</td>
            <td>${dist}</td>
...
function toggleLbHotStreak() {
    const section = document.getElementById('lb-hot-section');
    section.classList.toggle('collapsed');
    localStorage.setItem('lb-hot-collapsed', section.classList.contains('collapsed') ? '1' : '0');
}

(function initLbHotStreakCollapse() {
    const section = document.getElementById('lb-hot-section');
    if (!section) return;
    const saved = localStorage.getItem('lb-hot-collapsed');
    if (saved === '0') section.classList.remove('collapsed');
    else if (saved === '1') section.classList.add('collapsed');
})();

function toggleLbRecords() {
    const section = document.getElementById('lb-record-section');
    section.classList.toggle('collapsed');
    localStorage.setItem('lb-records-collapsed', section.classList.contains('collapsed') ? '1' : '0');
}

(function initLbRecordsCollapse() {
    const section = document.getElementById('lb-record-section');
    if (!section) return;
    const saved = localStorage.getItem('lb-records-collapsed');
    if (saved === '0') section.classList.remove('collapsed');
    else if (saved === '1') section.classList.add('collapsed');
})();
...
body {
    font-family: 'Inter', -apple-system, system-ui, sans-serif;
    background: var(--bg);
    background-image: radial-gradient(ellipse at 50% 0%, rgba(255,107,53,0.07) 0%, transparent 55%);
    color: var(--text);
    min-height: 100vh;
    padding: 20px;
}
...
.dot.on { background: var(--up); box-shadow: 0 0 8px rgba(34,197,94,0.5); }
.crawl-switch {
    display: inline-flex;
    align-items: center;
    gap: 8px;
...
.crawl-switch:hover { border-color: #64748b; }
.crawl-switch.on {
...
.crawl-switch.on .crawl-track { background: var(--up); }
.crawl-knob {
    position: absolute;
    top: 2px;
    left: 2px;
...
.crawl-switch.on .crawl-knob { left: 18px; }
.grid {
    display: grid;
...
.bar-up { background: var(--up); transition: width 0.3s; }
.bar-dn { background: var(--down); transition: width 0.3s; }
.shares {
    font-size: 12px;
    color: var(--text-muted);
    font-family: 'JetBrains Mono', monospace;
...
.hot-count.up { color: var(--up); }
.hot-count.dn { color: var(--down); }
.hot-bar {
    display: flex;
    width: 100%;
    height: 6px;
...
.hot-item.up { color: var(--up); }
.hot-item.dn { color: var(--down); }
.hot-item a { color: inherit; text-decoration: none; }
.hot-item a:hover { text-decoration: underline; }
.empty {
    color: var(--text-muted);
    font-size: 13px;
    padding: 8px 0;
...
<div class="app">
    <div class="top">
        <div class="brand">BTC <span>持仓看板</span></div>
        <div class="meta">
            <button type="button" class="crawl-switch" id="crawlSwitch" title="开启后开始链上爬取持仓数据">
                <span class="crawl-track"><span class="crawl-knob"></span></span>
                <span id="crawlLabel">爬取关闭</span>
            </button>
    ...
    <div class="grid">
        <div class="card" id="card-5m"></div>
        <div class="card" id="card-15m"></div>
        <div class="card" id="card-1h"></div>
    </div>
    <div class="foot">
        数据来自链上 Transfer 实时扫描 · YUCE BTC 看板
    </div>
</div>
<script>
let ws = null;
...
let pollTimer = null;
let lastData = null;
let serverOffset = 0; // server_time - Date.now()/1000
let connected = false;
...
function syncCrawlUI(enabled) {
    crawlEnabled = !!enabled;
    const el = document.getElementById('crawlSwitch');
    const lab = document.getElementById('crawlLabel');
    if (!el || !lab) return;
    el.classList.toggle('on', crawlEnabled);
    lab.textContent = crawlEnabled ? '爬取中' : '爬取关闭';
}

async function toggleCrawl() {
    if (crawlBusy) return;
    crawlBusy = true;
    const next = !crawlEnabled;
    syncCrawlUI(next); // 乐观更新
    try {
        ...
        if (data && data.success) {
            syncCrawlUI(data.enabled);
            if (connected) {
                setStatus(true, data.enabled ? '已连接 · 爬取中' : '已连接 · 爬取关闭');
            }
        } else {
            syncCrawlUI(!next);
        }
...
document.getElementById('crawlSwitch').addEventListener('click', (e) => {
    e.preventDefault();
    toggleCrawl();
});
...
function applyData(data) {
    ...
    if ('crawl_enabled' in data) syncCrawlUI(data.crawl_enabled);
    lastData = data;
    render(data);
}
...
async function pollState() {
    try {
        ...
        const data = await r.json();
        applyData(data);
        setStatus(true, crawlEnabled ? '已连接 · 爬取中' : '已连接 · 爬取关闭');
    } catch (err) {
        if (!connected) setStatus(false, '重连中…');
    }
}
...
function connectWs() {
    // 直连可用；经 YUCE iframe 代理时 WS 通常无法升级，主通道是 HTTP 轮询
    try {
...
function roundInfo(duration) {
    const now = nowSec();
    const ts = Math.floor(now / duration) * duration;
    const end = ts + duration;
    const remaining = Math.max(0, end - now);
    return { ts, end, remaining };
}
...
function hourlySlug(roundTs) {
    // ET = UTC-4 (与后端一致)
    const et = new Date((roundTs - 4 * 3600) * 1000);
...
function marketUrl(tf, roundTs) {
    let slug;
    if (tf === '1h') slug = hourlySlug(roundTs);
    else slug = `btc-updown-${tf}-${roundTs}`;
...
function pricesFor(data, tf) {
    const bp = (data && data.board_prices && data.board_prices[tf]) || {};
    // 5m 回退到交易面板当前盘口价
    if (tf === '5m' && !(bp.up > 0 || bp.down > 0) && data) {
        return { up: data.up_price || 0, down: data.down_price || 0 };
    }
    return { up: bp.up || 0, down: bp.down || 0 };
}

function updateOdds(el, tf, data) {
    const odds = el.querySelector('.odds');
    if (!odds) return;
    const p = pricesFor(data, tf);
    const upEl = odds.querySelector('.odds-up');
    const dnEl = odds.querySelector('.odds-dn');
    if (upEl) upEl.textContent = `UP ${fmtCents(p.up)}`;
    if (dnEl) dnEl.textContent = `DN ${fmtCents(p.down)}`;
}
...
const cardCache = {}; // tf -> { key, roundTs }

function cardKey(lb, roundTs) {
    if (!lb) return `empty:${roundTs}`;
    const hs = lb.hot_streak_stats || {};
    const hot = (lb.hot_streaks || []).map(h => `${h.addr}:${h.side}:${h.count}:${Math.round(h.shares)}`).join('|');
    return [
        roundTs,
        lb.up_count, lb.dn_count, lb.up_total, lb.dn_total, lb.scan_block,
        hs.up_n, hs.dn_n, hs.up_pct, hs.dn_pct, hot
    ].join('/');
}

function renderCard(el, label, tf, duration, lb, data) {
    const info = roundInfo(duration);
    const key = cardKey(lb, info.ts);
    const prev = cardCache[tf];
    if (prev && prev.key === key) {
        // 仅更新倒计时 / 盘口价，避免整卡重绘打断链接点击
        const timer = el.querySelector('.timer');
        if (timer) timer.setAttribute('data-end', String(info.end));
        updateOdds(el, tf, data);
        return;
    }
    cardCache[tf] = { key, roundTs: info.ts };

    const timerCls = info.remaining < 30 ? 'timer urgent' : 'timer';
    const url = marketUrl(tf, info.ts);
    const p = pricesFor(data, tf);
    const header = `
        <div class="card-h">
            <div class="tf">${label}</div>
            <div class="tf-sub">${fmtHHMM(info.ts)} → ${fmtHHMM(info.end)}</div>
        </div>
        <div class="card-meta">
            <div class="${timerCls}" data-end="${info.end}">⏱ ${fmtCountdown(info.remaining)}</div>
            <a class="pm-link" href="${url}" target="_blank" rel="noopener noreferrer">Polymarket ↗</a>
        </div>
        <div class="odds">
            <span class="odds-label">盘口</span>
            <span class="odds-up">UP ${fmtCents(p.up)}</span>
            <span class="odds-sep">/</span>
            <span class="odds-dn">DN ${fmtCents(p.down)}</span>
        </div>`;

    if (!lb || (lb.up_count == null && lb.dn_count == null)) {
        el.innerHTML = header + `<div class="empty">等待链上数据…</div>`;
        return;
    }
    const upN = lb.up_count || 0;
    const dnN = lb.dn_count || 0;
    const upPct = pct(upN, dnN);
    const dnPct = pct(dnN, upN);
    const arrow = upN > dnN ? '🟢' : dnN > upN ? '🔴' : '⚪';
    const upShares = Math.round(lb.up_total || 0).toLocaleString();
    const dnShares = Math.round(lb.dn_total || 0).toLocaleString();

    const hs = lb.hot_streak_stats || {};
    const hotItems = lb.hot_streaks || [];
    const hUp = hs.up_n || 0;
    const hDn = hs.dn_n || 0;
    const hUpPct = hs.up_pct || pct(hUp, hDn);
    const hDnPct = hs.dn_pct || pct(hDn, hUp);
    const hArrow = hUp > hDn ? '🟢' : hDn > hUp ? '🔴' : '⚪';

    let hotHtml = '';
    if (hotItems.length) {
        const items = hotItems.map(h => {
            const short = h.addr.slice(0, 6) + '…' + h.addr.slice(-4);
            const name = h.name ? ` (${h.name})` : '';
            const cls = h.side === 'UP' ? 'up' : 'dn';
            return `<span class="hot-item ${cls}">🔥${h.count}
                <a href="https://polymarket.com/profile/${h.addr}" target="_blank" rel="noopener noreferrer" title="${h.addr}">${short}${name}</a>
                ${h.side} ${Math.round(h.shares).toLocaleString()}股</span>`;
        }).join('');
        hotHtml = `
            <div class="hot-box">
                <div class="hot-h">
                    <div class="hot-title">🔥 连胜冒火 (${hotItems.length}人)</div>
                </div>
                <div class="hot-counts">
                    ${hArrow}
                    <span class="hot-count up">${hUp}人UP</span>
                    <span class="pct">(${hUpPct}%)</span>
                    <span class="vs">vs</span>
                    <span class="hot-count dn">${hDn}人DN</span>
                    <span class="pct">(${hDnPct}%)</span>
                </div>
                <div class="hot-bar">
                    <div class="bar-up" style="width:${hUpPct}%"></div>
                    <div class="bar-dn" style="width:${hDnPct}%"></div>
                </div>
                <div class="hot-items">${items}</div>
            </div>`;
    } else {
        hotHtml = `<div class="hot-box"><div class="hot-title">🔥 连胜冒火</div>
            <div class="empty" style="padding:8px 0">本轮暂无连胜地址</div></div>`;
    }

    el.innerHTML = header + `
        <div class="counts">
            <span class="count up">${upN}人UP</span>
            <span class="pct">(${upPct}%)</span>
            <span class="vs">vs</span>
            <span class="count dn">${dnN}人DN</span>
            <span class="pct">(${dnPct}%)</span>
        </div>
        <div class="bar">
            <div class="bar-up" style="width:${upPct}%"></div>
            <div class="bar-dn" style="width:${dnPct}%"></div>
        </div>
        <div class="shares">${arrow} 合计 ${upN + dnN} 人 · 股数 UP ${upShares} · DN ${dnShares}</div>
        ${hotHtml}`;
}

function render(data) {
    renderCard(document.getElementById('card-5m'), '5 分钟', '5m', 300, data.leaderboard, data);
    renderCard(document.getElementById('card-15m'), '15 分钟', '15m', 900, data.leaderboard_15m, data);
    renderCard(document.getElementById('card-1h'), '1 小时', '1h', 3600, data.leaderboard_1h, data);
    const lb = data.leaderboard || {};
    document.getElementById('block').textContent = lb.scan_block ? `区块 ${lb.scan_block}` : '';
...
function tickTimers() {
    document.querySelectorAll('.timer[data-end]').forEach(el => {
        ...
        const rem = Math.max(0, end - nowSec());
        el.textContent = `⏱ ${fmtCountdown(rem)}`;
        el.className = rem < 30 ? 'timer urgent' : 'timer';
        // 回合切换后强制刷新链接
        if (rem <= 0 && lastData) render(lastData);
    });
}
